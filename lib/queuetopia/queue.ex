defmodule PgQueuetopia.Queue do
  @moduledoc false
  ...
  @doc """
  List the available pending queues by scope a.k.a by PgQueuetopia.
  """
  ...
  @doc """
  Get the next available pending job of a given queue by scope a.k.a by PgQueuetopia.
  ...
  def get_next_pending_job(repo, scope, queue) when is_binary(queue) do
    job =
      ...
      |> where([j], j.scope == ^scope)
      |> where([j], is_nil(j.done_at))
      |> order_by(asc: :scheduled_at, asc: :id)
      |> limit(1)
      |> repo.one()
