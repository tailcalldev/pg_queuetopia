defmodule PgQueuetopia.SchedulerTest do
  ...
  test "poll only available queues" do
    scope = TestPgQueuetopia.scope()

    %Job{queue: queue} =
      insert!(:slow_job, params: %{"bin_pid" => pid_to_bin(), "duration" => 100}, scope: scope)
  ...
  test "poll only queues of its scope" do
    _scope = TestPgQueuetopia.scope()

    %Job{scope: _other_scope, queue: queue} =
      insert!(:slow_job, params: %{"bin_pid" => pid_to_bin(), "duration" => 100})

    start_supervised!({TestPgQueuetopia, poll_interval: 50})
  ...
  test "let a long job finish while the timeout is not reached" do
    ...
    %Job{queue: queue} =
      insert!(:slow_job,
        params: %{"bin_pid" => pid_to_bin(), "duration" => 200},
        scope: scope,
        timeout: 5_000
      )
  ...
  test "locks the queue during job processing" do
    ...
    %Job{queue: queue} =
      insert!(:slow_job,
        params: %{"bin_pid" => pid_to_bin(), "duration" => 300},
        timeout: 500,
        scope: scope
      )
  ...
  test "keeps the lock on the queue a while (one second) after the job timeouts" do
    ...
    %Job{queue: queue} =
      insert!(:slow_job,
        params: %{"bin_pid" => pid_to_bin(), "duration" => 5_000},
        timeout: 2_000,
        scope: scope
      )

    Application.put_env(:pg_queuetopia, TestPgQueuetopia, poll_interval: 500)
    start_supervised!(TestPgQueuetopia)
    ...
    test "a slow queue don't slow down the others" do
      ...
      %{queue: slow_queue, id: slow_job_id} =
        insert!(
          :slow_job,
          scope: scope,
          params: %{"bin_pid" => pid_to_bin(), "duration" => 500},
          timeout: 5_000
        )
  ...
  describe "failure persistence:" do
    ...
    test "an expired job persists the :timeout error and set the attempt attributes" do
      ...
      %{id: slow_job_id, queue: queue} =
        insert!(:slow_job,
          params: %{"bin_pid" => pid_to_bin(), "duration" => 100},
          timeout: 50,
          scope: scope
        )
